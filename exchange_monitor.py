...
from ma_signals import check_5m_signal, check_daily_signal
from config import GAINERS_REFRESH_SECONDS, DAILY_CHECK_INTERVAL_SECONDS
...
async def refresh_gainers_loop():
    """En çok yükselen coinleri periyodik olarak yeniden tarar."""
    while True:
        try:
            gainers = fetch_top_gainers()
            _state["top_gainer_symbols"] = [g["symbol"] for g in gainers]
            if gainers:
                top_list = ", ".join(f"{g['symbol']} (+{g['change_pct']:.1f}%)" for g in gainers[:5])
                print(f"[YÜKSELENLER] {len(gainers)} coin bulundu. İlk 5: {top_list}")
        except requests.RequestException as e:
            print(f"[HATA] Yükselen coin taraması başarısız: {e}")

        await asyncio.sleep(GAINERS_REFRESH_SECONDS)
...
async def ma_5m_signal_loop():
    """Her 5 dakikada bir, en çok yükselen ilk 20 coin için MA7/MA14 sinyalini kontrol eder."""
    while True:
        await asyncio.sleep(300)
        symbols = list(_state["top_gainer_symbols"])
        for symbol in symbols:
            await check_5m_signal(symbol)
            await asyncio.sleep(0.3)  # Bybit rate limit'e takılmamak için


async def daily_signal_loop():
    """Belirli aralıklarla, en çok yükselen ilk 20 coin için günlük MA50 durumunu kontrol eder."""
    while True:
        await asyncio.sleep(DAILY_CHECK_INTERVAL_SECONDS)
        symbols = list(_state["top_gainer_symbols"])
        for symbol in symbols:
            await check_daily_signal(symbol)
            await asyncio.sleep(0.3)


async def run_exchange_monitor():
    # İlk taramaları diğer görevler başlamadan önce senkron şekilde yap
    try:
        all_symbols = fetch_all_usdt_symbols_with_volume()
        high_vol, low_vol = classify_symbols(all_symbols)
        _state["high_volume_symbols"] = high_vol
    ...
    try:
        gainers = fetch_top_gainers()
        _state["top_gainer_symbols"] = [g["symbol"] for g in gainers]
        print(f"[BAŞLANGIÇ] {len(gainers)} yükselen coin bulundu.")
    except requests.RequestException as e:
        print(f"[HATA] Yükselen coin taraması başarısız: {e}")

    await asyncio.gather(
        refresh_universe_loop(),
        refresh_gainers_loop(),
        listen_trades(),
        analyze_flow_loop(),
