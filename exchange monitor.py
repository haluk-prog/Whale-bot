...
from config import GAINERS_REFRESH_SECONDS
...
async def ma_5m_signal_loop():
    """Her 5 dakikada bir, yüksek hacimli + en çok yükselen coinler için MA7 kesişimini kontrol eder."""
    while True:
        await asyncio.sleep(300)
        symbols = set(_state["high_volume_symbols"]) | set(_state["top_gainer_symbols"])
        for symbol in symbols:
...
async def ma_15m_signal_loop():
    """Her 15 dakikada bir, yüksek hacimli + en çok yükselen coinler için MA7/14/21 + MACD kademeli sinyalini kontrol eder."""
    while True:
