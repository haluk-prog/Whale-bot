...
# ---------------------------------------------------------------------------
# EN ÇOK YÜKSELEN COİNLER (24 saatlik % değişime göre)
# ---------------------------------------------------------------------------

def fetch_top_gainers(min_pct=GAINERS_MIN_PCT, limit=GAINERS_LIMIT):
    """
    Bybit'te 24 saatlik değişimi min_pct (%) üzerinde olan USDT çiftlerini bulur,
    en çok yükselenden başlayarak sıralar, ilk `limit` tanesini döndürür.
    Dönüş: [{"symbol": "XYZUSDT", "change_pct": 23.4}, ...]
    """
    ...
    for item in data.get("result", {}).get("list", []):
        ...
        if not symbol.endswith("USDT"):
            continue
        try:
            # Bybit yüzdeyi ondalık olarak verir (0.15 = %15)
